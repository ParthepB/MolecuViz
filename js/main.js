// Wait for both the DOM and script.js to be ready
        ...
        function getCurrentMolecule() {
            ...
            return activeIndex !== -1 ? moleculeDatabase[activeIndex] : null;
        }
        
        function showAIResponse(response, question) {
            const responseDiv = document.getElementById('aiResponse');
            const contentDiv = document.getElementById('aiResponseContent');
            
            contentDiv.innerHTML = `
                <div style="font-weight: bold; color: #4a5d8a; margin-bottom: 8px;">Q: ${question}</div>
        ...
        async function previewPDF() {
            ...
            try {
                const response = await fetch(`${AI_BASE_URL}/preview-pdf`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                alert('Could not generate PDF preview. Please make sure the server is running.');
            }
        }
        ...
        async function downloadPDF() {
            ...
            try {
                const response = await fetch(`${AI_BASE_URL}/generate-pdf`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (response.ok) {
                    ...
                    a.href = url;
                    a.download = `${molecule.name.replace(/\s+/g, '_')}_report.pdf`;
                ...
                alert('Could not download PDF. Please make sure the server is running.');
            }
        }